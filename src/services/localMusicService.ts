...
import { createRayIgnoreMatcher, isIgnoredByRayMatchers, type RayIgnoreMatcher } from '../utils/rayIgnore';
...
const LOCAL_MUSIC_UPDATED_EVENT = 'ray-local-music-updated';
export const LOCAL_MUSIC_SCAN_PROGRESS_EVENT = 'ray-local-music-scan-progress';
...
async function buildSnapshotTree(
    ...
    inheritedIgnoreMatchers: readonly RayIgnoreMatcher[],
    ...
    const ignoreHandle = childEntries.find(entry => entry.kind === 'file' && entry.name === '.rayignore');
    const localIgnoreMatcher = await loadRayIgnoreMatcher(
    ...
    for (const entry of childEntries) {
        ...
        const importRelativePath = entryPath.startsWith(`${rootFolderName}/`)
            ...
            : entryPath;
        if (isIgnoredByRayMatchers(ignoreMatchers, importRelativePath, entry.kind === 'directory')) {
            continue;
        }
...
async function loadRayIgnoreMatcher(
    ignoreHandle: FileSystemFileHandle | undefined,
    baseDirectory: string,
    directoryName: string,
): Promise<RayIgnoreMatcher> {
    if (!ignoreHandle) {
        return createRayIgnoreMatcher('', baseDirectory);
    }

    try {
        const ignoreFile = await ignoreHandle.getFile();
        const matcher = createRayIgnoreMatcher(await ignoreFile.text(), baseDirectory);
        console.log(`[LocalMusic][Import] Loaded ${matcher.ruleCount} .rayignore rules from "${directoryName}".`);
        return matcher;
    } catch (error) {
        console.warn(`[LocalMusic][Import] Failed to read .rayignore from "${directoryName}":`, error);
        return createRayIgnoreMatcher('', baseDirectory);
    }
}
