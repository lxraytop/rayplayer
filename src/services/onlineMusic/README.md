...
`omni.ts` 是 Ray Player 在线音乐服务层的**统一网络接口与调度中枢**。它向上层 UI 界面、React Hooks 及 Store 暴露标准化的在线音乐操作 API，向下屏蔽具体的在线音乐服务提供商（Provider，如网易云音乐 `Netease`、酷狗音乐 `KuGou` 等）的实现细节与接口差异。
