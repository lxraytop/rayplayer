// src/utils/rayIgnore.ts
// Parses root-level .rayignore rules and matches import-relative file-system paths.

interface RayIgnoreRule {
    negated: boolean;
    directoryOnly: boolean;
    matcher: RegExp;
}

export interface RayIgnoreMatcher {
...
const parseRule = (sourceLine: string): RayIgnoreRule | null => {
...
export const createRayIgnoreMatcher = (
    contents: string,
    baseDirectory = '',
): RayIgnoreMatcher => {
    const rules = contents
        ...
        .split('\n')
        .map(parseRule)
        .filter((rule): rule is RayIgnoreRule => Boolean(rule));
...
export const isIgnoredByRayMatchers = (
    matchers: readonly RayIgnoreMatcher[],
    relativePath: string,
    isDirectory: boolean,
): boolean => {
    let ignored = false;
