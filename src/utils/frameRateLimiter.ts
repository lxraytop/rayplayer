...
type WindowWithVisualizerFrameRateLimiter = Window & {
    __rayNativeRequestAnimationFrame?: RequestAnimationFrameFn;
    __rayNativeCancelAnimationFrame?: CancelAnimationFrameFn;
};
...
export const restoreGlobalVisualizerFrameRateLimiter = () => {
    ...
    if (frameWindow.__rayNativeRequestAnimationFrame) {
        window.requestAnimationFrame = frameWindow.__rayNativeRequestAnimationFrame;
    }
    if (frameWindow.__rayNativeCancelAnimationFrame) {
        window.cancelAnimationFrame = frameWindow.__rayNativeCancelAnimationFrame;
    }
...
export const installGlobalVisualizerFrameRateLimiter = (overrideFrameRate?: VisualizerFrameRate) => {
    ...
    const frameWindow = window as WindowWithVisualizerFrameRateLimiter;
    frameWindow.__rayNativeRequestAnimationFrame ??= window.requestAnimationFrame;
    frameWindow.__rayNativeCancelAnimationFrame ??= window.cancelAnimationFrame;
    const nativeRequestAnimationFrame: RequestAnimationFrameFn = (callback) => (
        frameWindow.__rayNativeRequestAnimationFrame?.call(window, callback) ?? 0
    );
    const nativeCancelAnimationFrame: CancelAnimationFrameFn = (handle) => {
        frameWindow.__rayNativeCancelAnimationFrame?.call(window, handle);
    };
