...
// The shareable appearance config codec: theme + visual settings <-> the minified `ray-theme://`
...
export const compressConfig = (config: any): string => {
    ...
    return `ray-theme://${base64}`;
};

/**
 * Decodes and restores a configuration object from either raw JSON or a compressed base64 string starting with 'ray-theme://'.
 */
export const decompressConfig = (str: string): any => {
    let parsed: any = null;
    const trimmed = str.trim();
    const isCompressedShortcode = trimmed.startsWith('ray-theme://');
    if (isCompressedShortcode) {
        const base64 = trimmed.slice('ray-theme://'.length);
