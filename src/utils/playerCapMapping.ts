...
// Pure mapping from PlayerCap events to Ray shapes (black-box consumption, no side effects, unit-testable).
...
//     with the progress clock, matching PlayerCap's display timing — no need to recompute offset on the Ray side.
//   - 'timestamp': lines/words use the raw timestamp (on-beat, ignores PlayerCap offset; can be fine-tuned via Ray's own offset control).
...
// A single PlayerCap lyric line → Ray Line. Word-by-word lines use the words' timing; line-level lines degrade to one Word for the whole line.
...
function buildLine(line: PlayerCapLyricLine, basis: PlayerCapTimeBasis, startTime: number, nextStart: number | undefined): Line {
  ...
  if (detailed) {
    ...
    // Line-level lines have no internal duration: cap at Ray's native parseLRC reading duration (text.length×0.5+2,
...
export function buildLyricLines(data: PlayerCapAllLyricsData, basis: PlayerCapTimeBasis = DEFAULT_TIME_BASIS): Line[] {
  ...
  // credit lines use Ray's existing lyric-filtering regex.
...
// song_info_update → Ray track snapshot. Read name/singer directly for title/artist; do not parse title.
