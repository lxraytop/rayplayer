...
export const buildSonnetStaffView = (
    ...
    const beatWidth = playableWidth / LA_RAY_TOTAL_BEATS;
    const timedNotes: TimedSonnetStaffNote[] = [];
    let beatCursor = 0;
    for (const note of LA_RAY_STAFF_NOTES) {
    ...
    /** Draws the fixed La Ray phrase and advances its playback cursor on a normal time loop. */
    const updateAnimation = (time: number) => {
        const cycleElapsed = positiveModulo(time - shotStartTime, LA_RAY_CYCLE_SECONDS);
        const beatPosition = (cycleElapsed / LA_RAY_CYCLE_SECONDS) * LA_RAY_TOTAL_BEATS;
