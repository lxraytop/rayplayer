...
import { useRayHexViewport } from './ray-grid/useRayHexViewport';
import {
    applyHexCardFrameStyles,
    computeHexCardFrame,
    createHexCardFrameStyleCache,
    type HexCardFrameStyleCache,
} from './ray-grid/hexCardTransform';
...
} from './ray-grid/progressiveGrid';
import { useProgressiveItemEntrance } from './ray-grid/useProgressiveItemEntrance';
...
import { resolveGridViewContextTracks } from './ray-grid/gridViewContextActions';
import {
...
const GRID_VIEW_NAVIGATION_PREFIX = 'ray_gridview_state';
const GRID_VIEW_LAST_INDEX_PREFIX = 'ray_gridview_last_index';
...
export const GridView: React.FC<GridViewProps> = ({
    ...
    const fetchRemainingTracks = async (
        ...
        // @note Version Control: Project Ray version 0.5.27-a16525c
    ...
    const handleToggleSubscribe = async () => {
        ...
        try {
            ...
            if (isOnlinePlaylist || isOnlineAlbum) {
                ...
                if (isOnlineAlbum) {
                    window.dispatchEvent(new CustomEvent('ray-refresh-favorite-albums'));
                }
    ...
    const memoizedCards = useMemo(() => {
        return renderedIndexes.map((idx) => {
            ...
            return (
                <div
                    key={`${mode}-${item.id}`}
                    data-ray-grid-item-id={String(item.id)}
                    ref={(el) => {
                        if (el) {
                        ...
                        if (cardWrapperRefs.current[idx]?.dataset.rayGridItemId === String(item.id)) {
                            cardWrapperRefs.current[idx] = null;
                            cardFrameStyleCachesRef.current[idx] = undefined;
                        }
                    }}
                    className="absolute select-none pointer-events-auto ray-grid-card-frame"
                    style={{
