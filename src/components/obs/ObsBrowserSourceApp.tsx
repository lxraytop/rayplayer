...
// Read-only OBS browser source renderer driven by Ray's main playback clock.
    ...
    if (!config) {
        return (
            <div className="h-screen w-screen bg-transparent grid place-items-center text-white/70 text-sm">
                {connected ? t('obs.waitingForPlayback', 'Waiting for Ray playback') : t('obs.connecting', 'Connecting to Ray')}
            </div>
        );
    }
