...
const RAY_GUIDE_URL = 'https://ray-site.lxray.top/guide/llm-routing';
const RAY_DOCS_URL = 'https://ray-site.lxray.top/guide/';
...
export const AiHelpPromptModal: React.FC<AiHelpPromptModalProps> = ({
    ...
    const prompt = useMemo(() => t('aiHelp.prompt', {
        guideUrl: RAY_GUIDE_URL,
        repoUrl: RAY_REPOSITORY_URL,
        defaultValue: [
            'I am using Ray and need help with a problem.',
            '',
            `Ray Guide: ${RAY_GUIDE_URL}`,
            `Ray repository: ${RAY_REPOSITORY_URL}`,
            '',
                        ...
                        <div className="relative z-10 space-y-5">
                            <div className="pr-10">
                                ...
                                <p className={`mt-2 text-sm leading-6 ${textSecondary}`}>
                                    {t('aiHelp.description', 'Check the Ray documentation first. If the issue remains unresolved, copy the prompt below and ask an AI model for help.')}
                                </p>
                            </div>
                            ...
                            >
                                ...
                                <span className="min-w-0 flex-1">
                                    <span className="block text-sm font-semibold">{t('aiHelp.docsTitle', 'Read the Ray documentation')}</span>
                            ...
                            <div>
                                ...
                                <p className={`mt-1 text-xs leading-5 ${textSecondary}`}>
                                    {t('aiHelp.askAiDescription', 'Describe your issue, then paste this prompt so the AI can use Ray documentation and source code as context.')}
                                </p>
                            </div>
