...
const linuxGraphicsMode =
  process.platform !== 'linux'
    ? 'system'
    : (process.env.RAY_LINUX_GRAPHICS_MODE || (isAppImageRuntime ? 'swiftshader' : 'system'));
...
const store = new Store({ projectName: 'Ray' });
...
// would let clicks reach the KDE desktop window, which KWin then raises above Ray (both are
...
// The wrapped child keeps RAY_WRAPPED_BY_WINDOWTOLAYER=1; it must never wrap itself again.
...
// RAY_WINDOWTOLAYER_PATH overrides it for non-packaged (dev) runs; the dev:electron* scripts
...
function resolveWindowToLayerPath() {
  const override = process.env.RAY_WINDOWTOLAYER_PATH;
  if (override) {
...
// Enables wallpaper mode on Wayland: spawn windowtolayer wrapping a fresh Ray child, then the
...
function launchWrappedSelf({ onError } = {}) {
  ...
  return new Promise((resolve) => {
    const child = spawn(wtl, ['--layer=bottom', '--interactivity=all',
      ...
      {
        env: { ...process.env, RAY_WRAPPED_BY_WINDOWTOLAYER: '1', RAY_RELAUNCH: '1' },
        stdio: 'inherit',
      });
...
// windowtolayer; the wrapped child uses RAY_RELAUNCH to retry after the jumpboard exits.
...
const RAY_RELEASES_URL = 'https://github.com/lxraytop/rayplayer/releases';
const RAY_GITHUB_REPOSITORY = {
  owner: 'lxraytop',
  repo: 'rayplayer',
};
const WINDOWS_APP_USER_MODEL_ID = 'top.lxray.player';
const REMOTE_CONTROL_WINDOW_TITLE = 'Ray Remote';
...
// the overlay setup where Ray sits on top of whatever else is on screen and takes no clicks.
...
// (RAY_RELAUNCH=1): the old instance has just called app.exit() and is about to
// release the lock, so a fresh process may need a few attempts before it wins it.
function acquireSingleInstanceLock() {
  if (app.requestSingleInstanceLock()) {
    return true;
  }
  if (process.env.RAY_RELAUNCH !== '1') {
...
// Compares two semver version strings (e.g. "1.5.0" vs "1.4.9").
// Returns 1 if left > right, -1 if left < right, 0 if equal or unparsable.
...
function normalizeUpdateChannelSelection(value) {
  ...
  return channel === 'stable' ? channel : null;
}
...
function isDevUpdatePreviewEnabled() {
  return process.env.ELECTRON_DEV === 'true' && process.env.RAY_DEV_UPDATE_PREVIEW === 'true';
}
...
function setupAutoUpdater() {
  ...
  updater.on('update-available', (info) => {
    ...
    const currentVersion = normalizeVersion(app.getVersion());
    // Defensive check: only surface the update if the remote version is actually newer.
    ...
    setUpdateState({
      ...
      updateUrl: getReleaseUrl(getCurrentReleaseChannel().id, version, RAY_RELEASES_URL),
      error: null,
      lastCheckedAt: Date.now(),
      downloadProgress: null,
    });
...
function configureAutoUpdaterChannel(updater) {
  ...
  const providerConfig = getUpdateProviderConfig(releaseChannel, RAY_GITHUB_REPOSITORY);
  if (providerConfig) {
...
async function checkForUpdates({ manual = false } = {}) {
  if (isDevUpdatePreviewEnabled()) {
    ...
    setUpdateState({
      status: 'available',
      availableVersion,
      updateUrl: getReleaseUrl(getCurrentReleaseChannel().id, availableVersion, RAY_RELEASES_URL),
      error: null,
      lastCheckedAt: Date.now(),
      downloadProgress: null,
    });
...
async function openUpdateReleasePage(version) {
  ...
  const url = normalizedVersion
    ? getReleaseUrl(getCurrentReleaseChannel().id, normalizedVersion, RAY_RELEASES_URL)
    : updateState.updateUrl || RAY_RELEASES_URL;
...
function setMainWindowClickThroughEnabled(enabled) {
  ...
  // would raise it above Ray (both desktop-type), covering the wallpaper. The state stays off.
...
async function getMainWindowCaptureSource() {
  ...
  const source =
    ...
    sources.find(item => item.name.toLowerCase().includes('ray') && item.name !== REMOTE_CONTROL_WINDOW_TITLE) ||
    null;
