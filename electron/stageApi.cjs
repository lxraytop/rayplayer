...
// one parser-compatible lyrics session, push one media session, or ask Ray
...
function createStageApi({
  ...
  const requestStageSongPlay = async (songId, options = {}) => {
    const mainWindow = getMainWindow();
    if (!mainWindow || mainWindow.isDestroyed()) {
      throw new StageApiError('Ray main window is unavailable for external play requests.', {
        statusCode: 503,
  ...
  const requestStagePlayerRendererAction = async (channel, pendingRequests, requestPrefix, payload, unavailableCode) => {
    const mainWindow = getMainWindow();
    if (!mainWindow || mainWindow.isDestroyed()) {
      throw new StageApiError('Ray main window is unavailable for Stage player requests.', {
        statusCode: 503,
