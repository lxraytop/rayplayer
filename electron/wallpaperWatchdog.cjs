...
//
// Process model: a "jumpboard" Ray spawns windowtolayer (wtl); wtl spawns the real wrapped
// Ray child with WAYLAND_SOCKET=<fd>. The wrapped child's parent is wtl. If wtl dies the
...
function isWallpaperWrapped(env) {
  return env.RAY_WRAPPED_BY_WINDOWTOLAYER === '1';
}
...
function createWallpaperWatchdog(options = {}) {
  ...
  // Relaunches Ray as a plain (non-wrapped) process without touching the stored mode.
  function relaunchSelfNormal() {
    const childEnv = { ...env, RAY_RELAUNCH: '1' }; // marker lets the new process win the lock race
    delete childEnv.WAYLAND_SOCKET;
    delete childEnv.RAY_WRAPPED_BY_WINDOWTOLAYER;
