...
// Ray's KuGou and QQ repositories refuse `basic_text` on purpose, so those users silently lose
...
/**
 * Returns the `--password-store` value Ray should append, or `null` to leave Chromium's own
 * detection untouched.
 */
function resolveLinuxPasswordStore({ platform = process.platform, env = process.env, argv = process.argv } = {}) {
  ...
  const override = String(env.RAY_PASSWORD_STORE || '').trim();
  if (override) {
