...
歌词接口是 Ray Electron 桌面端提供的本机只读 HTTP API。外部程序可以通过它获取 Ray 当前加载的歌词数据，无需鉴权。

## 基本信息

| 项目 | 值 |
| --- | --- |
| 可用平台 | Ray Electron 桌面端 |
| 协议 | HTTP |
| 监听地址 | `127.0.0.1` |
| 固定端口 | `32109` |
| API 版本 | `v1` |
...
也可以从命令面板执行“歌词接口”命令进行切换。设置会被持久化；启用后，Ray 下次启动时会自动尝试监听固定端口。
...
| `wordByWord` | `boolean` | 是 | 表示时间轴来源：`true` 为数据源提供的原生逐字时间，`false` 为 Ray 根据逐行时间合成的逐字时间。两种情况下歌词行都会提供 `words`。 |
...
| `words` | `LyricWord[]` | 是 | 当前行的逐字时间轴。原始数据只有逐行时间时，由 Ray 歌词流水线自动拆分文本并合成逐字时间，因此正常歌词行不会因为源格式是 LRC/VTT 等逐行格式而返回空数组。 |
...
即使原始歌词只提供行开始时间，Ray 也会在进入统一歌词流水线时生成逐字时间。此时 `wordByWord` 为 `false`，用于说明这些逐字时间是估算值，而不是数据源提供的精确逐字时间。
...
接口不会直接暴露 Ray 内部的完整渲染对象。当前 `v1` 使用以下转换规则：
...
客户端不应依赖 JSON 字段的排列顺序，也不应假设未来版本会公开 Ray 的内部字段。
...
当接口未启用、Ray 未运行或端口被占用时，客户端会遇到连接失败，不会收到 HTTP 状态码。
...
- Ray 会把逐行歌词归一化为逐字时间轴；`wordByWord: false` 表示逐字时间由行时间估算生成，外部程序不应把它当作原生精确逐字时间。
