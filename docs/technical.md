...
- [Ray Guide](https://ray-site.lxray.top/guide/)
...
Realeco 发布仅在 `main` 推送版本标签时触发。自动触发时，该文件必须只包含单行 `A.B.C`，并且必须同时与 `package.json` 版本、当前完整提交信息 `release: vA.B.C` 严格一致。手动触发作为应急入口，会跳过这两项一致性检查，直接使用当前 HEAD 的 `package.json.version`（仍必须是稳定的 `A.B.C`）打包。工作流创建草稿 Release；维护者在 GitHub 手动公开后，Realeco 客户端才会发现更新。
...
如果你希望快速上线 Web 版本，可以在 Vercel 中导入本仓库创建项目。
