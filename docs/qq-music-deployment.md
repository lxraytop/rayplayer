...
这篇指南会带你为 Ray Web 版启用 QQ 音乐，包括登录、歌单、收藏专辑、我喜欢和播放功能。
...
- 如果要在 Cloudflare 使用 QQ 扫码登录，Ray 必须使用包含 `@yakult-green-tea/qq-music-api` 3.1.0 或更高版本的代码。
...
| 配置项 | 用途 | 是否必需 |
| --- | --- | --- |
| `VITE_QQ_API_BASE` | Ray 前端请求 QQ 音乐 API 的地址 | Web 版需要 |
...
使用 Ray 内置的 Vercel 或 Cloudflare 接口时，`VITE_QQ_API_BASE` 固定填写：
...
登录 Vercel，选择自己的 GitHub 账号，然后导入 Ray 仓库。
...
如果返回 Ray HTML 页面，不要把 HTTP 200 当作成功，应先检查仓库根目录的 `vercel.json` 是否仍包含 `/api/qq/:path*` rewrite，并确认当前 Deployment 来自包含 QQ serverless 入口的分支。
...
```bash
git clone https://github.com/lxraytop/rayplayer.git
cd ray-player
npm install
```
...
Ray 会自动显示 QQ 和微信两个选项，不需要修改前端代码。
...
Ray 的完整 Docker 堆栈已经包含 `qq-api`，不需要单独安装 npm 包：
...
| 现象 | 优先检查 |
| --- | --- |
| 页面显示 `Login Error` | `/api/qq/login/channels` 是否返回 JSON，而不是 Ray HTML 或平台登录页 |
