...
// Stand-in for the wrapped Ray process used by wallpaper-watchdog-sim.ts. Plain Node, no
...
const watchdog = createWallpaperWatchdog({
  ...
  spawnFn: (cmd, args, opts) => {
    append(
      ...
      `RAY_RELAUNCH=${opts.env.RAY_RELAUNCH} ` +
      `WAYLAND_SOCKET=${opts.env.WAYLAND_SOCKET ?? ''} ` +
      `WRAPPED=${opts.env.RAY_WRAPPED_BY_WINDOWTOLAYER ?? ''}`
    );
    return {
