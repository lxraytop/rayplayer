...
本文档描述桌面端本地 Stage API 的 HTTP / WebSocket 契约。默认服务地址为 `http://127.0.0.1:32107`，实际端口以 Ray 设置中的 Stage 端口为准。
...
| `modeEnabled` | `boolean` | Ray 舞台模式是否启用。 |
...
| `audioSrc` | `string` | Ray 实际播放地址；URL 模式等于 `audioUrl`，文件模式为本地 Stage media URL。 |
...
把搜索请求转交给 Ray 当前接入的搜索通道，返回可供点播接口消费的候选结果。
...
| `503` | `STAGE_PLAY_UNAVAILABLE` | Ray 主窗口不可用。 |
...
读取 Ray 播放器状态。该接口返回队列摘要，不返回完整队列 `items`。
...
| `503` | `STAGE_PLAYER_CONTROL_UNAVAILABLE` | Ray 主窗口不可用或 Stage 服务停止。 |
...
| `503` | `STAGE_PLAYER_QUEUE_UNAVAILABLE` | Ray 主窗口不可用或 Stage 服务停止。 |
| `504` | `STAGE_PLAYER_REQUEST_TIMEOUT` | 播放器 10 秒内未完成响应。 |
