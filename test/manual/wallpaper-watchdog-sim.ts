...
//
// Expected status sequence: `started` → `relaunch-spawned ... RAY_RELAUNCH=1 WAYLAND_SOCKET= WRAPPED=` → `exit 0`.

const __dirname = path.dirname(fileURLToPath(import.meta.url));
const statusFile = path.join(os.tmpdir(), `ray-watchdog-sim-${process.pid}.log`);
  ...
  try {
    ...
    const recovered = await waitFor(
      (status) =>
        ...
        status.includes('RAY_RELAUNCH=1') &&
