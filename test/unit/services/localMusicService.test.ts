...
describe('localMusicService', () => {
    ...
    it('applies root .rayignore rules to files, snapshots, and nested directories', async () => {
        const selectedHandle = new FakeDirectoryHandle('Music', [
            new FakeFileHandle('.rayignore', {
                content: 'Ignored/\n*.tmp.mp3\n!keep.tmp.mp3\n',
                type: 'text/plain',
            }),
    ...
    it('applies nested .rayignore rules relative to each directory', async () => {
        const selectedHandle = new FakeDirectoryHandle('Music', [
            new FakeFileHandle('.rayignore', { content: '*.mp3\n', type: 'text/plain' }),
            new FakeDirectoryHandle('Album', [
                new FakeFileHandle('.rayignore', { content: '!keep.mp3\n*.flac\n', type: 'text/plain' }),
