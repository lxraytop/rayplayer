...
describe('localCoverAssetUrl', () => {
    ...
    it('returns a stable Electron protocol URL for a valid SHA-256 asset id', () => {
        ...
        expect(getLocalCoverAssetUrl(assetId)).toBe(`ray-cover://asset/${encodeURIComponent(assetId)}`);
        expect(getLocalCoverAssetUrl(assetId, 320)).toBe(`ray-cover://asset/${encodeURIComponent(assetId)}?size=512`);
