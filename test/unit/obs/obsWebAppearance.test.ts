...
describe('parseObsWebParams', () => {
    it('reads host/cfg/daylight/transparent/visualizer with OBS defaults', () => {
        const p = parseObsWebParams('?host=localhost%3A9863&cfg=ray-theme%3A%2F%2Fabc&daylight=1&visualizer=cadenza');
        expect(p.host).toBe('localhost:9863');
        expect(p.cfg).toBe('ray-theme://abc');
...
describe('extractCfgFromInput', () => {
    it('pulls cfg out of a full OBS URL', () => {
        const url = 'https://example.test/?obs=1&obsSource=now-playing&host=localhost%3A9863&cfg=ray-theme%3A%2F%2Fabc123';
        expect(extractCfgFromInput(url)).toBe('ray-theme://abc123');
    });

    it('passes through a bare shortcode or raw JSON', () => {
        expect(extractCfgFromInput('ray-theme://abc')).toBe('ray-theme://abc');
...
describe('buildObsSourceUrl', () => {
    it('bakes source + host + cfg into the query', () => {
        const url = buildObsSourceUrl('now-playing', 'ray-theme://abc', 'localhost:9863');
        expect(url).toContain('obs=1');
        expect(url).toContain('obsSource=now-playing');
        expect(url).toContain('host=localhost%3A9863');
        expect(url).toContain('cfg=ray-theme');
        // round-trip：从组装的 URL 再剥回 cfg。
        expect(extractCfgFromInput(url.startsWith('http') ? url : `https://x.test${url}`)).toBe('ray-theme://abc');
    });

    it('carries extra params (daylight/transparent) ahead of the terminal cfg', () => {
        const url = buildObsSourceUrl('now-playing', 'ray-theme://abc', '', { daylight: '1', transparent: '0' });
    ...
    it('keeps the long cfg blob last, after source-specific extra params', () => {
        const url = buildObsSourceUrl('playercap', 'ray-theme://abc', 'lan:8765', { nxpcPlayer: 'cloudmusicv3', nxpcBasis: 'timestamp' });
        ...
        expect(extractCfgFromInput(`https://x.test${url}`)).toBe('ray-theme://abc');
    });
});
