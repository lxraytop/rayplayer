...
describe('parseMicrophoneConsentStoreInUse', () => {
    it('detects an actively capturing app (stop == 0 with a real start)', () => {
        ...
        expect(parseMicrophoneConsentStoreInUse(output, 'C:\\Apps\\Ray.exe')).toBe(true);
    });
    ...
    it('ignores apps whose capture session already ended', () => {
        ...
        expect(parseMicrophoneConsentStoreInUse(output, 'C:\\Apps\\Ray.exe')).toBe(false);
    });
    ...
    it('excludes the Ray process itself from NonPackaged entries', () => {
        const output = buildRegOutput([
            { subKey: 'NonPackaged\\C:#Program Files#Ray#Ray.exe', start: '0x1dc1234abcd0000', stop: '0x0' },
        ]);

        expect(parseMicrophoneConsentStoreInUse(output, 'C:\\Program Files\\Ray\\Ray.exe')).toBe(false);
    });

    it('still reports other apps capturing alongside the Ray process', () => {
        const output = buildRegOutput([
            { subKey: 'NonPackaged\\C:#Program Files#Ray#Ray.exe', start: '0x1dc1234abcd0000', stop: '0x0' },
            { subKey: 'NonPackaged\\C:#IME#sogou#SogouVoice.exe', start: '0x1dc1234abcd0000', stop: '0x0' },
        ]);

        expect(parseMicrophoneConsentStoreInUse(output, 'C:\\Program Files\\Ray\\Ray.exe')).toBe(true);
    });

    it('treats empty or malformed output as not in use', () => {
        expect(parseMicrophoneConsentStoreInUse('', 'C:\\Apps\\Ray.exe')).toBe(false);
        expect(parseMicrophoneConsentStoreInUse('ERROR: The system was unable to find the specified registry key or value.', 'C:\\Apps\\Ray.exe')).toBe(false);
    });
});
