...
describe('localCoverAssets', () => {
    it('places local-library covers directly under userData', () => {
        const userDataDirectory = path.join('C:', 'Users', 'tester', 'Ray');
    ...
    it('accepts only the supported clear thumbnail sizes', () => {
        expect(parseThumbnailSize(new URL('ray-cover://asset/id?size=512'))).toBe(512);
        expect(parseThumbnailSize(new URL('ray-cover://asset/id?size=1024'))).toBe(1024);
        expect(parseThumbnailSize(new URL('ray-cover://asset/id?size=256'))).toBeNull();
    });
});
