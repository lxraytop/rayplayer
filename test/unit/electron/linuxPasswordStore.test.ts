...
describe('resolveLinuxPasswordStore', () => {
    ...
    it('never overrides an explicit launch flag', () => {
        expect(resolve({ XDG_CURRENT_DESKTOP: 'Hyprland' }, ['ray', '--password-store=basic'])).toBeNull();
        expect(resolve({ XDG_CURRENT_DESKTOP: 'Hyprland' }, ['ray', '--password-store', 'kwallet'])).toBeNull();
    });

    it('honours a supported RAY_PASSWORD_STORE override and ignores unknown values', () => {
        expect(resolve({ XDG_CURRENT_DESKTOP: 'Hyprland', RAY_PASSWORD_STORE: 'kwallet6' })).toBe('kwallet6');
        expect(resolve({ XDG_CURRENT_DESKTOP: 'Hyprland', RAY_PASSWORD_STORE: 'auto' })).toBeNull();
        expect(resolve({ XDG_CURRENT_DESKTOP: 'Hyprland', RAY_PASSWORD_STORE: 'nonsense' })).toBeNull();
    });
