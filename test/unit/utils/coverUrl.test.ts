...
describe('coverUrl utilities', () => {
    ...
    it('uses bounded thumbnail buckets for Electron local covers', () => {
        expect(getSizedCoverUrl(`ray-cover://asset/sha256%3A${'a'.repeat(64)}`, 300))
            .toBe(`ray-cover://asset/sha256%3A${'a'.repeat(64)}?size=512`);
    });

    it('keeps Web local cover thumbnails at least 512px', () => {
        expect(getSizedCoverUrl(`/__ray_cover/sha256%3A${'b'.repeat(64)}`, 50))
            .toBe(`/__ray_cover/sha256%3A${'b'.repeat(64)}?size=512`);
    });
});
