...
这是 Ray 的官方同步服务端实现。本服务以“多端同构”为目标设计，底层逻辑完全一致。
...
| --- | --- | --- | --- | --- |
| `SYNC_TOKEN` | 用于客户端鉴权。Ray 客户端必须拥有此 Token 才能读取和覆盖同步数据。 | **必填** | 8 ~ 32 位随机字符 | 重新生成并覆盖环境变量 |
...
- 安装项目依赖
- 调用 `npx wrangler d1 create ray-sync -c wrangler.toml` 创建 D1 数据库
...
```bash
npx wrangler d1 create ray-sync -c wrangler.toml
```
...
```
启动后，服务监听容器内的 `3000` 端口，并映射到宿主机的 `13000` 端口；数据库文件会持久化保存在 `sync-server/data/ray-sync.db`。
...
# 选填：SQLite 数据库保存路径（默认在当前目录生成 ray-sync.db）
DB_PATH="./ray-sync.db"
```
...
Ray 客户端的“存储设置”使用 `workerBaseUrl` 和 `SYNC_TOKEN` 连接服务端。地址填写服务根地址，例如：

```text
https://ray-sync.example.workers.dev
